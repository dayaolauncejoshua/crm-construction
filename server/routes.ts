...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/vsls", async (req, res) => {
    try {
      ...
      res.status(500).json({
        message:
          error instanceof Error ? error.message : "Failed to create VSL",
      });
    }
  });
  ...
  async function generateVideoAsync(
    ...
    try {
      ...
      const { videoUrl, thumbnailUrl, duration } =
        await vslGenerator.generateVSL({
          script,
          title,
  ...
  // Get single VSL
  app.get("/api/vsls/:clientId/:vslId", async (req, res) => {
    try {
  ...
  // Update VSL
  app.patch("/api/vsls/:vslId", async (req, res) => {
    try {
      const { vslId } = req.params;
  ...
  // Delete VSL
  app.delete("/api/vsls/:vslId", async (req, res) => {
    try {
      const { vslId } = req.params;
