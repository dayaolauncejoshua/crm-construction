...
export default function SOPs() {
  ...
  const createVideoSOPMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/video-sops", selectedClientId] });
  ...
  const createNotionSOPMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/notion-sops", selectedClientId] });
  ...
  const syncNotionMutation = useMutation({
    mutationFn: async () => {
      const response = await apiRequest("POST", `/api/notion-sops/${selectedClientId}/sync`, {});
      return response.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/notion-sops", selectedClientId] });
  ...
  const mockVideoSOPs = [
    {
      ...
      description: "Complete guide on configuring and optimizing lead capture forms for maximum conversion",
      category: "onboarding",
      videoUrl: "https://example.com/video1.mp4",
      thumbnailUrl: "https://via.placeholder.com/320x180/3b82f6/ffffff?text=Lead+Forms",
    ...
    },
    {
      ...
      description: "Common issues and solutions when integrating WhatsApp Business API",
      category: "troubleshooting",
      videoUrl: "https://example.com/video2.mp4",
      thumbnailUrl: "https://via.placeholder.com/320x180/10b981/ffffff?text=WhatsApp+Fix",
    ...
    },
    {
      ...
      description: "Deep dive into analytics features and creating custom reports",
      category: "advanced",
      videoUrl: "https://example.com/video3.mp4",
      thumbnailUrl: "https://via.placeholder.com/320x180/f59e0b/ffffff?text=Analytics",
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-slate-600">Loading SOPs...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white border-b border-slate-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-slate-900">Standard Operating Procedures</h2>
              <p className="text-slate-600">Video SOPs and Notion documentation for your team</p>
            </div>
            <div className="flex items-center space-x-3">
              ...
              >
                <Sync className={`w-4 h-4 mr-2 ${syncNotionMutation.isPending ? 'animate-spin' : ''}`} />
                Sync Notion
              </Button>
              ...
              <Dialog open={showCreateNotionDialog} onOpenChange={setShowCreateNotionDialog}>
                <DialogTrigger asChild>
        ...
        <main className="flex-1 overflow-auto p-6">
            ...
            <TabsContent value="video" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">{mockVideoSOPs.length}</div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {mockVideoSOPs.reduce((sum, sop) => sum + sop.viewCount, 0).toLocaleString()}
                    </div>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {Math.round(mockVideoSOPs.reduce((sum, sop) => sum + sop.duration, 0) / 60)} min
                    </div>
                  ...
                  <Card key={sop.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between mb-2">
                        <h3 className="font-medium text-lg line-clamp-2">{sop.title}</h3>
                        <Badge className={getCategoryColor(sop.category)}>
                          {sop.category}
                        </Badge>
                      </div>
                      <p className="text-sm text-slate-600 line-clamp-2 mb-3">{sop.description}</p>
            ...
            <TabsContent value="notion" className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">{mockNotionSOPs.length}</div>
                    <p className="text-xs text-muted-foreground">Synced from Notion</p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-green-600">
                      {mockNotionSOPs.filter(sop => sop.syncStatus === 'active').length}
                    </div>
                      ...
                      <div key={sop.id} className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="flex items-center space-x-4">
                          ...
                          <div>
                            ...
                            <div className="flex items-center space-x-4 text-sm text-slate-600 mt-1">
                              ...
                              <span className="flex items-center">
                                <CheckCircle className={`w-3 h-3 mr-1 ${getSyncStatusColor(sop.syncStatus)}`} />
                                {sop.syncStatus}
                              </span>
                              <span>
                                Last synced: {new Date(sop.lastSynced).toLocaleDateString()}
                              </span>
                            </div>
                          </div>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Button variant="outline" size="sm" asChild>
                            <a href={sop.pageUrl} target="_blank" rel="noopener noreferrer">
                              <ExternalLink className="w-3 h-3 mr-1" />
                              Open in Notion
                            </a>
                          </Button>
          ...
          <Dialog open={showCreateVideoDialog} onOpenChange={setShowCreateVideoDialog}>
            <DialogContent className="sm:max-w-[600px]">
              ...
              <Form {...videoForm}>
                <form onSubmit={videoForm.handleSubmit(onSubmitVideoSOP)} className="space-y-4">
                  <FormField
                  ...
                  <div className="flex justify-end space-x-3 pt-4">
                    ...
                    >
                      {createVideoSOPMutation.isPending ? "Creating..." : "Create Video SOP"}
                    </Button>
                  </div>
                </form>
              </Form>
          ...
          <Dialog open={showCreateNotionDialog} onOpenChange={setShowCreateNotionDialog}>
            <DialogContent className="sm:max-w-[500px]">
              ...
              <Form {...notionForm}>
                <form onSubmit={notionForm.handleSubmit(onSubmitNotionSOP)} className="space-y-4">
                  <FormField
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>Category</FormLabel>
                        <FormControl>
                          <Input placeholder="e.g., onboarding, processes" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="https://notion.so/your-page-id" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Extract from URL or use page ID" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <div className="flex justify-end space-x-3 pt-4">
                    ...
                    <Button
                      ...
                      className="bg-primary text-white hover:bg-primary/90"
                    >
                      {createNotionSOPMutation.isPending ? "Linking..." : "Link Notion SOP"}
                    </Button>
                  </div>
                </form>
              </Form>
